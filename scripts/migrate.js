...
async function migrate() {
  ...
  try {
    ...
    `
    console.log("✓ Tabela ativos_categorias criada")
    ...
    await sql`
      DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ativos_codigo_unique') THEN
          ALTER TABLE ativos ADD CONSTRAINT ativos_codigo_unique UNIQUE(codigo);
        END IF;
      END $$
    `
    await sql`CREATE INDEX IF NOT EXISTS idx_ativos_categoria_id ON ativos (categoria_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_ativos_status ON ativos (status)`
    ...
    `
    await sql`CREATE INDEX IF NOT EXISTS idx_ativos_tipos_vistoria_categoria_id ON ativos_tipos_vistoria (categoria_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_ativos_tipos_vistoria_periodicidade ON ativos_tipos_vistoria (periodicidade)`
    console.log("✓ Tabela ativos_tipos_vistoria criada")
    ...
    `
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS ativos_planos_ativo_tipo_unique ON ativos_planos_vistoria (ativo_id, tipo_vistoria_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_ativos_planos_vistoria_proxima_data ON ativos_planos_vistoria (proxima_data)`
    console.log("✓ Tabela ativos_planos_vistoria criada")
    ...
    `
    await sql`CREATE INDEX IF NOT EXISTS idx_ativos_vistorias_status ON ativos_vistorias (status)`
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_tickets_prioridade ON tickets (prioridade)`
    await sql`CREATE INDEX IF NOT EXISTS idx_tickets_area_id ON tickets (area_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_tickets_solicitante_id ON tickets (solicitante_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_tickets_responsavel_id ON tickets (responsavel_id)`
    await sql`CREATE INDEX IF NOT EXISTS idx_tickets_created_at ON tickets (created_at)`
    ...
    `
    await sql`CREATE INDEX IF NOT EXISTS idx_ticket_mensagens_ticket_id ON ticket_mensagens (ticket_id)`

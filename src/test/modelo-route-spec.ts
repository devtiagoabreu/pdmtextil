...
export function modeloRouteSpec(cfg: ModeloRouteSpecConfig) {
  ...
  describe(title, () => {
    ...
    it("retorna 401 quando não há sessão", async () => {
      ...
      const res = await GET(new NextRequest("http://localhost/api/modelo"))
    ...
    it("retorna CSV quando formato=csv", async () => {
      vi.mocked(getServerSession).mockResolvedValue(mockSession(roles?.[0]) as any)
      const res = await GET(new NextRequest("http://localhost/api/modelo?formato=csv"))
      expect(res.status).toBe(200)
    ...
    it("retorna JSON quando formato=json", async () => {
      vi.mocked(getServerSession).mockResolvedValue(mockSession(roles?.[0]) as any)
      const res = await GET(new NextRequest("http://localhost/api/modelo?formato=json"))
      expect(res.status).toBe(200)

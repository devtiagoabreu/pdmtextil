...
export default function ImportarFornecedores({ onImportado }: ImportarFornecedoresProps) {
  ...
  const baixarModeloCSV = async () => {
    try {
      const res = await fetch("/api/cadastros/fornecedores/modelo?formato=csv")
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = "fornecedores_modelo.csv"
      a.click()
  ...
  const baixarModeloJSON = async () => {
    try {
      const res = await fetch("/api/cadastros/fornecedores/modelo?formato=json")
      const dados = await res.json()
      const blob = new Blob([JSON.stringify(dados, null, 2)], { type: "application/json" })
      const url = URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = "fornecedores_modelo.json"
      a.click()
      URL.revokeObjectURL(url)
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      console.log("[ImportarFornecedores] Arquivo selecionado:", file.name)
  ...
  const handleImportar = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/cadastros/fornecedores/importar", {
        method: "POST",
        body: formData,
      })

      const dados = await res.json()
      console.log("[ImportarFornecedores] Resposta:", dados)
      ...
      console.error("[ImportarFornecedores] Erro:", error)

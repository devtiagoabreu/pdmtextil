...
export default function ImportarFios({ onImportado }: ImportarFiosProps) {
  ...
  const baixarModeloCSV = async () => {
    try {
      const res = await fetch("/api/cadastros/fios/modelo?formato=csv")
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = "fios_modelo.csv"
      a.click()
  ...
  const baixarModeloJSON = async () => {
    try {
      const res = await fetch("/api/cadastros/fios/modelo?formato=json")
      const dados = await res.json()
      const blob = new Blob([JSON.stringify(dados, null, 2)], { type: "application/json" })
      const url = URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = "fios_modelo.json"
      a.click()
      URL.revokeObjectURL(url)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-lg rounded-xl bg-white dark:bg-slate-900 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 p-4">
              <h2 className="text-lg font-semibold">Importar Fios</h2>
              <button

...
export function ViagemForm({ form, setField, investimentos, setInvestimentos }: ViagemFormProps) {
  function updateInvestimento(index: number, campo: keyof InvestimentoLinha, valor: string) {
    setInvestimentos(
      investimentos.map((inv, i) => (i === index ? { ...inv, [campo]: valor } : inv))
    )
  }
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
        ...
        {investimentos.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400 py-2">
            Nenhum investimento registrado. Clique em &quot;Adicionar&quot; para incluir passagens,
          ...
          <div className="space-y-3">
            {investimentos.map((inv, index) => (
              <div
                key={index}
                className="grid grid-cols-12 gap-3 items-center rounded-lg border border-slate-200 dark:border-slate-700 p-3"
              >

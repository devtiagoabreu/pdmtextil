...
const ViagemRotaMapa = dynamic(() => import("./viagem-rota-mapa"), {
  ssr: false,
  loading: () => (
    <div className="h-64 w-full animate-pulse bg-slate-100 dark:bg-slate-800 rounded-xl" />
  ),
})
...
export default function ViagemCronogramaModal({
  viagemId,
  viagemTitulo,
  open,
  onClose,
}: ViagemCronogramaModalProps) {
  const { data, isLoading } = useQuery<ViagemCronogramaData>({
    queryKey: ["viagem-cronograma", viagemId],
    queryFn: () =>
      fetch(`/api/crm/visitas/dashboard/viagem?viagemId=${viagemId}`).then((r: any) => r.json()),
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <Chip
                  ...
                  label="Km total"
                  value={formatKm(resumo?.kmTotal ?? 0)}
                  tone="text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-950/50"
                />
                <Chip
                  icon={<Calendar size={14} />}
                  label="Visitas"
                  value={`${resumo?.total ?? 0}`}
                  tone="text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800/60"
                />
                <Chip
                  icon={<CheckCircle2 size={14} />}
                  label="Realizadas"
                  value={`${resumo?.realizadas ?? 0}`}
                  tone="text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-950/50"
                />
                <Chip
                  icon={<MapPin size={14} />}
                  label="Com localização"
                  value={`${resumo?.comLocalizacao ?? 0}`}
              ...
              {resumo && resumo.geocodificadas + resumo.comEndereco > 0 && (
                <p className="text-[11px] text-amber-600 dark:text-amber-400">
                  Localização de {resumo.geocodificadas + resumo.comEndereco} visita(s) estimada
                  pelo endereço (sem check-in).
                </p>
              )}
                      ...
                      <li key={v.id} className="relative pl-9 pb-5 last:pb-0">
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="flex flex-col items-end gap-1 shrink-0">
                            <span
                              className={`text-[10px] px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${STATUS_BADGES[v.status] || STATUS_BADGES.AGENDADA}`}
                            >

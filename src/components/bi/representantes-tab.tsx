...
function RepRankingTable({ reps }: { reps: any[] }) {
  if (!reps.length)
    return (
      <p className="text-sm text-slate-500 py-4 text-center">Nenhum representante no período.</p>
    )
  return (
          ...
          {reps.map((r, i) => (
            ...
            >
              ...
              <td className="py-2 px-2 text-right text-slate-500">
                {r.totalQtd.toLocaleString("pt-BR")}
              </td>
              <td className="py-2 px-2 text-right text-slate-500">
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          label="Faturamento Média/Rep"
          value={reps.length ? totalVendas / reps.length : 0}
          icon={Medal}
          prefix="R$ "
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {painel(
          "5 Menores Faturamento",
          <AlertTriangle className="w-4 h-4 text-red-500" />,
          reps.slice(-5).reverse(),
          "totalVendas",
          fmtValor,
          "Sem dados no período"
        )}

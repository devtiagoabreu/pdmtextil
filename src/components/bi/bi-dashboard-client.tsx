...
export function BiDashboardClient() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "produto" | "grupo">("dashboard")
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
                  ...
                  <div className="relative mb-3">
                    ...
                    <input
                      ...
                      placeholder="Buscar grupo (ex: K1820, T0093)..."
                      className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                  </div>
                  <div className="max-h-[400px] overflow-y-auto space-y-1">
                  ...
                  <p className="text-xs text-slate-400 mt-2">{grupoList.length} grupos no total</p>
                </div>
              </div>
              ...
              <div className="lg:col-span-2">
                {!selectedGrupo && (
                  <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 text-center">
                    ...
                    <p className="text-sm text-slate-500">Selecione um grupo ao lado para ver os clientes</p>
                  </div>
                )}
                ...
                {selectedGrupo && !loadingClientes && (
                  <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
                    <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">
                      Clientes que compraram do grupo: <span className="text-indigo-600 dark:text-indigo-400">{selectedGrupo}</span>
                    </h3>
                ...
                {selectedGrupo && clientesData.length > 0 && (
                  <ChartCard title={`Top Clientes - Grupo ${selectedGrupo}`} delay={100}>
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={clientesData.slice(0, 10)} layout="vertical" margin={{ left: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                        <XAxis type="number" tick={{ fontSize: 10 }} stroke="#94a3b8" />

      ...
      {sheetData.metrics && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <StatCard
            ...
            value={sheetData.metrics.totalVendas}
            icon={TrendingUp}
            prefix="R$ "
          />
          <StatCard
            label="Representantes"
            value={sheetData.metrics.totalRepresentantes}
            icon={Layers}
          />
          <StatCard
            label="Clientes Atendidos"
            value={sheetData.metrics.totalClientes}
            icon={ShoppingCart}
          />
          <StatCard label="Linhas (Itens)" value={sheetData.metrics.totalLinhas} icon={BarChart3} />
          <StatCard
            label="Previsão do Mês"
            value={sheetData.previsao?.projecaoMes ?? 0}
            icon={CalendarClock}
            prefix="R$ "
          />
        </div>
              ...
              >
                {(sheetData.abcCurve || []).map((item: any) => (
                  <Cell
                    key={item.grupo}
                    fill={
                      item.classe === "A" ? "#ef4444" : item.classe === "B" ? "#f97316" : "#22c55e"
                    }
                  />
                ))}
              </Bar>
            ...
            {(() => {
              ...
              const itens = [
                ...
                { label: "Projeção do mês atual", value: p.projecaoMes ?? 0, money: true },
                {
                  ...
                  value: p.projecaoProximos30 ?? 0,
                  money: true,
                },
                {
                  label: "Projeção próximo mês (média 3m)",
                  value: p.projecaoProximoMes ?? 0,
                  money: true,
                },
              ...
              return itens.map((x) => (
                <div
                  key={x.label}
                  className="flex items-center justify-between text-sm border-b border-slate-100 dark:border-slate-800 last:border-0 pb-2 last:pb-0"
                >

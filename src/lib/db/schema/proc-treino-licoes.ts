...
export const procTreinoLicoes = pgTable("proc_treino_licoes", {
  ...
  moduloId: integer("modulo_id")
    .notNull()
    .references(() => procTreinoModulos.id, { onDelete: "cascade" }),
  titulo: varchar("titulo", { length: 200 }).notNull(),
  conteudoMd: text("conteudo_md").notNull().default(""),
  preRequisitos: text("pre_requisitos"),
  linksPop: jsonb("links_pop")
    .$type<{ label: string; url: string; descricao?: string | null }[]>()
    .default([]),
  linksVideo: jsonb("links_video")
    .$type<{ label: string; url: string; descricao?: string | null }[]>()
    .default([]),

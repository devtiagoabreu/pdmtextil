...
export const chats = pgTable(
  ...
  (t: any) => [index("idx_chats_entidade").on(t.entidadeTipo, t.entidadeId)]
)
...
export const chatMensagens = pgTable(
  ...
  (t: any) => [index("idx_chat_mensagens_chat").on(t.chatId)]
)
...
export const chatParticipantes = pgTable(
  ...
  (t: any) => [index("idx_chat_participantes_usuario").on(t.usuarioId)]
)

export const chatLeituras = pgTable("chat_leituras", {
  id: serial("id").primaryKey(),
  mensagemId: integer("mensagem_id")
    .references(() => chatMensagens.id, { onDelete: "cascade" })
    .notNull(),
  usuarioId: integer("usuario_id")
    .references(() => usuarios.id, { onDelete: "cascade" })
    .notNull(),

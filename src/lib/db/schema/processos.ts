...
export const procSites = pgTable("proc_sites", {
  ...
  empresaId: integer("empresa_id")
    .notNull()
    .references(() => procEmpresas.id, { onDelete: "cascade" }),
  nome: varchar("nome", { length: 200 }).notNull(),
...
export const procAreas = pgTable("proc_areas", {
  ...
  siteId: integer("site_id")
    .notNull()
    .references(() => procSites.id, { onDelete: "cascade" }),
  nome: varchar("nome", { length: 200 }).notNull(),
...
export const procProcessos = pgTable("proc_processos", {
  ...
  areaId: integer("area_id")
    .notNull()
    .references(() => procAreas.id, { onDelete: "cascade" }),
...
export const procSubprocessos = pgTable("proc_subprocessos", {
  ...
  processoId: integer("processo_id")
    .notNull()
    .references(() => procProcessos.id, { onDelete: "cascade" }),
  nome: varchar("nome", { length: 200 }).notNull(),
  descricao: text("descricao"),
...
export const procAtividades = pgTable("proc_atividades", {
  id: serial("id").primaryKey(),
  subprocessoId: integer("subprocesso_id")
    .notNull()
    .references(() => procSubprocessos.id, { onDelete: "cascade" }),
  nome: varchar("nome", { length: 200 }).notNull(),

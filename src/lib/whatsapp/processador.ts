...
export async function executarFluxo(req: NextRequest, filaId?: number | null) {
  ...
  if (filaId) {
    if (res.status === 202) {
      await marcarFilaStatus(filaId, "PENDENTE")
...
async function executarFluxoInterno(req: NextRequest) {
  ...
  try {
    ...
    try {
      const res = await processarConversa({
        executionId,
        remoteJid,
        pushName,
        mensagem,
        isNew: aquisicao.criada,
        leadExistente: aquisicao.leadExistente,
        lockToken: aquisicao.token,
      })
      return res
    } finally {
...
async function processarConversa(params: {
  ...
  const { executionId, remoteJid, pushName, mensagem, isNew, leadExistente, lockToken } = params

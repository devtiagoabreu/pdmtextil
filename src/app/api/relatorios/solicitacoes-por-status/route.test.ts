...
describe("GET /api/relatorios/solicitacoes-por-status", () => {
  ...
  it("retorna 401 quando não está autenticado", async () => {
    ...
    const res = await GET(
      new NextRequest("http://localhost/api/relatorios/solicitacoes-por-status")
    )
  ...
  it("retorna as estatísticas com o shape esperado", async () => {
    ...
    const res = await GET(
      new NextRequest("http://localhost/api/relatorios/solicitacoes-por-status?status=PENDENTE")
    )

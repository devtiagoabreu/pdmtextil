...
describe("GET /api/relatorios/amostras-por-status", () => {
  ...
  it("retorna 401 quando não está autenticado", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/relatorios/amostras-por-status"))
  ...
  it("retorna as estatísticas com o shape esperado", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/relatorios/amostras-por-status?status=PENDENTE"))

import { NextRequest, NextResponse } from "next/server"
import { getServerSession } from "next-auth"
import { authOptions } from "@/lib/auth"
import { db } from "@/lib/db"
import { coresSolidas } from "@/lib/db/schema/cores"
import { eq } from "drizzle-orm"
import { handleApiError } from "@/lib/api-error"
import { notificarDelecao } from "@/lib/notificar"

export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const session = await getServerSession(authOptions)
    if (!session) return NextResponse.json({ error: "Não autorizado" }, { status: 401 })

    const id = parseInt((await params).id)
    const cor = await db.select().from(coresSolidas).where(eq(coresSolidas.id, id)).limit(1)

    if (!cor[0]) {
      return NextResponse.json({ error: "Cor não encontrada" }, { status: 404 })
    }

    return NextResponse.json(cor[0])
  } catch (error) {
    console.error("[GET /api/cadastros/cores/[id]]", error)
    return NextResponse.json({ error: "Erro interno do servidor" }, { status: 500 })
  }
}

export async function PUT(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const session = await getServerSession(authOptions)
    if (!session) return NextResponse.json({ error: "Não autorizado" }, { status: 401 })

    const id = parseInt((await params).id)
    const body = await req.json()

    if (body.idIntegracao) {
      const existenteIdInt = await db
        .select()
        .from(coresSolidas)
        .where(eq(coresSolidas.idIntegracao, body.idIntegracao))
        .limit(1)

      if (existenteIdInt[0] && existenteIdInt[0].id !== id) {
        return NextResponse.json({ error: "ID Integração já cadastrado em outra cor" }, { status: 409 })
      }
    }

    const corAtualizada = await db
      .update(coresSolidas)
      .set({
        nome: body.nome,
        pantone: body.pantone || null,
        familia: body.familia || null,
        ativo: body.ativo ?? true,
        idIntegracao: body.idIntegracao || null,
      })
      .where(eq(coresSolidas.id, id))
      .returning()

    if (!corAtualizada[0]) {
      return NextResponse.json({ error: "Cor não encontrada" }, { status: 404 })
    }

    return NextResponse.json(corAtualizada[0])
  } catch (error) {
    console.error("[PUT /api/cadastros/cores/[id]]", error)
    return NextResponse.json({ error: "Erro ao atualizar cor" }, { status: 500 })
  }
}

export async function DELETE(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const session = await getServerSession(authOptions)
    if (!session) return NextResponse.json({ error: "Não autorizado" }, { status: 401 })

    const id = parseInt((await params).id)
    await db.delete(coresSolidas).where(eq(coresSolidas.id, id))

    await notificarDelecao("Cor", id.toString(), session?.user?.name)

    return NextResponse.json({ success: true })
  } catch (error) {
    return handleApiError(error, "DELETE /api/cadastros/cores/[id]")
  }
}
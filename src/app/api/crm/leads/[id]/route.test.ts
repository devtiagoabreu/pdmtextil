...
function del(id: string) {
  return DELETE(new NextRequest(`http://localhost/api/crm/leads/${id}`), {
...
describe("DELETE /api/crm/leads/[id]", () => {
  ...
  it("exclui o lead e suas oportunidades em transação", async () => {
    const res = await del("8")
    expect(res.status).toBe(200)
    expect(await res.json()).toEqual({ success: true })
    expect(db.transaction).toHaveBeenCalled()
    expect(notificarDelecao).toHaveBeenCalledWith("Lead CRM", "8", "Tiago")
  })

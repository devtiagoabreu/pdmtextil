...
function del(id: string) {
  return DELETE(new NextRequest(`http://localhost/api/crm/tarefas/${id}`), {
...
describe("DELETE /api/crm/tarefas/[id]", () => {
  beforeEach(() => {
  ...
  it("exclui a tarefa e limpa os eventos de timeline em transação", async () => {
    const res = await del("3")
    expect(res.status).toBe(200)
    expect(await res.json()).toEqual({ success: true })
    expect(db.transaction).toHaveBeenCalled()
    expect(notificarDelecao).toHaveBeenCalledWith("Tarefa CRM", "3", "Tiago")
  })

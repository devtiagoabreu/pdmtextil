...
export async function GET(req: NextRequest) {
  try {
    ...
    let whereStatus = ""
    if (filtro === "solicitados") whereStatus = "AND r.status = 'SOLICITADO'"
    else if (filtro === "processando") whereStatus = "AND r.status = 'PROCESSANDO'"
    else if (filtro === "atendidos") whereStatus = "AND r.status = 'ATENDIDO'"
    else if (filtro === "este-mes") {
      const now = new Date()
      const start = new Date(now.getFullYear(), now.getMonth(), 1).toISOString()
      const end = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59).toISOString()
      whereStatus = `AND r.created_at >= '${start}'::timestamp AND r.created_at <= '${end}'::timestamp`
    }

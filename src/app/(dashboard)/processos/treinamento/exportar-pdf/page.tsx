...
export default function ExportarPdfPage() {
  ...
  const { data: modulos, isLoading } = useQuery<ModuloCompleto[]>({
    queryKey: ["proc-treinamento-exportar-pdf"],
    queryFn: () => fetch("/api/processos/treinamento/exportar-pdf").then((r) => r.json()),
  })
  ...
  return (
    <>
      <style>{`
        ...
        .pdf-content blockquote { border-left: 3px solid #0ea5e9; padding-left: 12px; color: #444; }
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="no-print flex items-center gap-4 mb-6">
          ...
          <button
            ...
            disabled={exportando}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-sky-600 text-white text-sm rounded-lg hover:bg-sky-700 disabled:opacity-50 transition-colors"
          >

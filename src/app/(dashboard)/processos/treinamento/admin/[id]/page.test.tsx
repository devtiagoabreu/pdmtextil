...
const licao = {
  ...
  preRequisitos: "Cadastro de empresa",
...
describe("EditarLicaoPage", () => {
  ...
  it("carrega e exibe os dados da lição", async () => {
    ...
    await screen.findByDisplayValue("O que é um processo")
    expect(screen.getByDisplayValue("Cadastro de empresa")).toBeInTheDocument()
  ...
  it("salva as alterações via PUT e navega para a lista", async () => {
    ...
    fireEvent.change(screen.getByDisplayValue("O que é um processo"), {
      target: { value: "O que é um processo v2" },
    })
    fireEvent.submit(container.querySelector("form")!)

    await waitFor(() => {
      const call = findCall(fetchMock.calls, "/api/processos/treinamento/10", "PUT")
      expect(call).toBeDefined()
      expect(call!.body).toEqual({
        moduloId: 1,
        titulo: "O que é um processo v2",
        conteudoMd: "## Visão Geral\n\nConteúdo da lição.",
        preRequisitos: "Cadastro de empresa",
        linksPop: [{ label: "POP", url: "https://exemplo.com/pop" }],
        linksVideo: [],
    ...
    expect(navMock.router.push).toHaveBeenCalledWith("/processos/treinamento/admin")
  })
})
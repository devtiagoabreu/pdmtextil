...
export default function AdminTreinamentoPage() {
  ...
  const deleteLicao = useMutation({
    mutationFn: (id: number) => fetch(`/api/processos/treinamento/${id}`, { method: "DELETE" }),
    onSuccess: () => {
  ...
  const criarModulo = useMutation({
    mutationFn: (data: typeof novoModulo) =>
      fetch("/api/processos/treinamento/modulos", {
        method: "POST",
    ...
    onSuccess: () => {
      ...
      setNovoModulo({ titulo: "", descricao: "", icone: "GraduationCap", cor: "#0ea5e9" })
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-600 text-white text-sm rounded-lg hover:bg-sky-700 transition-colors"
        >
          <Plus size={16} />
          Novo Módulo
        </button>

...
function NovaLicaoPageContent() {
  ...
  const { data: modulos } = useQuery<Modulo[]>({
    queryKey: ["proc-treinamento-modulos"],
    queryFn: () => fetch("/api/processos/treinamento/modulos").then((r) => r.json()),
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/processos/treinamento/admin")
    } catch {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800"
              placeholder="Ex: Cadastro de Processos - Campo a Campo"
              required
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800"
              placeholder="/processos/processos"
            />
          </div>
        </div>

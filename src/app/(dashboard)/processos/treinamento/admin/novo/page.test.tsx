...
describe("NovaLicaoPage", () => {
  ...
  it("cria a lição via POST e navega para a lista", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Ex: Cadastro de Processos - Campo a Campo"), {
      target: { value: "Cadastro de Processos" },
    })
    fireEvent.submit(container.querySelector("form")!)

    await waitFor(() => {
      const call = findCall(fetchMock.calls, "/api/processos/treinamento", "POST")

...
export default function ProcessoProcessosPage() {
  ...
  const filtered = processos.filter((p: ProcessoDto) => matchesSearch(p, search))
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/processos/processos/${deleteTarget.id}`, { method: "DELETE" })
      ...
      toast.error(err instanceof Error ? err.message : "Erro ao excluir processo")
      setDeleteTarget(null)
    } finally {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Buscar por nome, área, código ou responsável..."
            value={search}
              ...
              {filtered.map((proc: ProcessoDto) => (
                ...
                >
                  ...
                  <td className="p-4 text-sm text-slate-500">
                    {[proc.codigo, proc.areaNome, proc.siteNome].filter(Boolean).join(" · ") || "—"}
                  </td>

      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-slate-500">
            Nenhum tipo de vistoria encontrado
          </div>
        ) : (
              ...
              {filtered.map((tipo: TipoVistoria) => (
                ...
                >
                  <td className="p-4 text-sm font-medium">
                    <Link href={`/ativos/tipos-vistoria/${tipo.id}`}>
                      {tipo.nome}
                    </Link>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-sm text-slate-500 line-clamp-1">{tipo.baseLegal || "—"}</td>
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Link href={`/ativos/tipos-vistoria/${tipo.id}`} onClick={(e) => e.stopPropagation()}>

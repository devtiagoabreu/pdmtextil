...
export default function FornecedoresPage() {
  ...
  const filteredFornecedores = fornecedores.filter((f: Fornecedor) => matchesSearch(f, search))
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/cadastros/fornecedores/${deleteTarget.id}`, {
        method: "DELETE",
      })
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <ImportarEntidade
            config={{
              titulo: "Fornecedores",
              apiBase: "cadastros/fornecedores",
              arquivoPrefixo: "fornecedores",
            }}
            onImportado={() => refetch()}
          />
          <ExportarDados
              ...
              {filteredFornecedores.map((fornecedor: Fornecedor) => (
                ...
                >
                  <td className="p-4 text-sm font-medium">
                    <Link href={`/cadastros/fornecedores/${fornecedor.id}`}>{fornecedor.nome}</Link>
                  </td>

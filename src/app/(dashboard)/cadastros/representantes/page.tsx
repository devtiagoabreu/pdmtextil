...
async function fetchRepresentantes(): Promise<Representante[]> {
  const res = await fetch("/api/cadastros/representantes")
  if (!res.ok) throw new Error("Falha ao carregar representantes")
  return res.json()
}
...
export default function RepresentantesPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Representante | null>(null)
  ...
  const filteredRepresentantes = representantes.filter((c: Representante) =>
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/cadastros/representantes/${deleteTarget.id}`, {
        method: "DELETE",
      })
      ...
      toast.error(err instanceof Error ? err.message : "Erro ao excluir representante")
      setDeleteTarget(null)
    } finally {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-slate-500">Nenhum representante encontrado</div>
        ) : (
              ...
              {filteredRepresentantes.map((r: Representante) => (
                ...
                >
                  <td className="p-4 text-sm font-medium">
                    <Link href={`/comercial/representantes/${r.id}`}>{r.nome}</Link>
                  </td>

...
import { PlusCircle, ShoppingCart, Search, ChevronLeft, ChevronRight, Trash2, Pencil, FileText, Package } from "lucide-react"
import { toast } from "sonner"
import { PageSkeleton } from "@/components/ui/page-skeleton"
import { STATUS_PEDIDO_VENDA_LABELS, STATUS_PEDIDO_VENDA_CORES } from "@/lib/crm/documento-venda"
import type { PedidoVendaResumo } from "./types"
...
type PedidosVendaPage = {
  data: PedidoVendaResumo[]
  total: number
  page: number
  totalPages: number
  limit: number
...
async function fetchPedidosVendaPaginated(params: { page: number; q: string; status: string }) {
  const sp = new URLSearchParams()
  sp.set("page", String(params.page))
  sp.set("limit", String(PAGE_SIZE))
  if (params.q) sp.set("q", params.q)
  if (params.status && params.status !== "all") sp.set("status", params.status)
  const res = await fetch(`/api/crm/pedidos-venda?${sp}`)
  if (!res.ok) throw new Error("Falha ao carregar")
  return res.json() as Promise<PedidosVendaPage>
}

function PedidosVendaPageContent() {
  ...
  const [excluirPedido, setExcluirPedido] = useState<PedidoVendaResumo | null>(null)
  ...
  const { data: tableData, isLoading } = useQuery<PedidosVendaPage>({
    queryKey: ["crm-pedidos-venda", page, debouncedSearch, statusFilter],
    queryFn: () => fetchPedidosVendaPaginated({ page, q: debouncedSearch, status: statusFilter }),
    retry: 1,
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await fetch(`/api/crm/pedidos-venda/${id}`, { method: "DELETE" })
      if (!res.ok) {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["crm-pedidos-venda"] })
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">Pedidos de Venda{info && <InfoButton content={info} />}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            {isLoading ? "Carregando..." : totalRows > 0 ? `${fromRow}-${toRow} de ${totalRows} pedido(s)` : "0 pedidos"}
          </p>
        </div>
        <Link
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          {Object.entries(STATUS_PEDIDO_VENDA_LABELS).map(([value, label]) => (
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <ShoppingCart className="w-12 h-12 text-slate-300 dark:text-slate-700 mb-3" />
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {debouncedSearch ? "Nenhum pedido encontrado para essa busca" : "Nenhum pedido de venda encontrado"}
            </p>
            <Link href="/comercial/crm/pedidos-venda/novo" className="text-sm text-blue-600 hover:underline mt-2">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {rows.map((p) => (
                    <tr
                      key={p.id}
                      onClick={() => router.push(`/comercial/crm/pedidos-venda/${p.id}`)}
                    ...
                    >
                      ...
                      <td className="px-2 py-2 md:px-4 md:py-3 text-xs md:text-sm text-slate-500 dark:text-slate-400 hidden sm:table-cell">
                        {p.oportunidadeTitulo || `Oportunidade #${p.oportunidadeId}`}
                      </td>
                      ...
                      <td className="px-2 py-2 md:px-4 md:py-3">
                        <span className={`inline-flex text-[10px] px-1.5 md:px-2 py-0.5 rounded-full font-medium ${STATUS_PEDIDO_VENDA_CORES[p.status] || ""}`}>
      ...
      <ConfirmModal
        open={!!excluirPedido}
        title="Excluir pedido de venda"
        message={`Tem certeza que deseja excluir o pedido${excluirPedido?.numero ? ` "${excluirPedido.numero}"` : ` #${excluirPedido?.id}`}?`}
        subMessage="Os itens vinculados também serão removidos."
        confirmLabel="Excluir"
        loading={deleteMutation.isPending}
        onConfirm={() => excluirPedido && deleteMutation.mutate(excluirPedido.id)}
        onCancel={() => setExcluirPedido(null)}
      />
    </div>
  )
}

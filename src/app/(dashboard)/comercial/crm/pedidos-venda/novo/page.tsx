...
function NovoPedidoVendaContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const res = await fetch("/api/crm/pedidos-venda", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "Erro ao criar pedido de venda")
      }
      const pedido = await res.json()
      toast.success("Pedido de venda criado")
      router.push(`/comercial/crm/pedidos-venda/${pedido.id}`)
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Erro ao criar pedido de venda")
    } finally {
      setSaving(false)
    }
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {saving ? "Salvando..." : "Criar Pedido de Venda"}
          </button>
          <button
            type="button"
            onClick={() => router.back()}

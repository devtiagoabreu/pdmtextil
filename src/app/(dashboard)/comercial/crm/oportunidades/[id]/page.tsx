...
const PROPOSTA_STATUS: Record<string, { label: string; cor: string }> = {
  ENVIADA: {
    label: "Enviada",
    cor: "text-blue-600 bg-blue-50 dark:bg-blue-950/50 dark:text-blue-400",
  },
  ACEITA: {
    label: "Aceita",
    cor: "text-green-600 bg-green-50 dark:bg-green-950/50 dark:text-green-400",
  },
  RECUSADA: {
    label: "Recusada",
    cor: "text-red-600 bg-red-50 dark:bg-red-950/50 dark:text-red-400",
  },
  REVISAO: {
    label: "Em Revisão",
    cor: "text-amber-600 bg-amber-50 dark:bg-amber-950/50 dark:text-amber-400",
  },
}
            ...
            {editingStatus ? (
              <div className="flex items-center gap-1.5">
                ...
                <button
                  onClick={() => {
                    setEditingStatus(false)
                    setStatusValue(oportunidade.status)
                  }}
              ...
              <button onClick={() => setEditingStatus(true)} className="group relative">
                <span
                  ...
                  style={{
                    color: currentStatus?.cor || "#64748b",
                    backgroundColor: `${currentStatus?.cor || "#64748b"}20`,
                  }}
                >
                ...
                <Pencil
                  size={12}
                  className="absolute -top-1 -right-2 opacity-0 group-hover:opacity-100 text-slate-400"
                />
              </button>
            )}
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          ...
          <div className="space-y-3 text-sm">
            ...
            <Field
              label="Probabilidade"
              value={oportunidade.probabilidade != null ? `${oportunidade.probabilidade}%` : "—"}
            />
            <Field
              label="Previsão"
              value={
                oportunidade.dataFechamentoPrevista
                  ? new Date(oportunidade.dataFechamentoPrevista).toLocaleDateString("pt-BR")
                  : "—"
              }
            />
            ...
            <Field
              ...
              value={oportunidade.empresaNome || oportunidade.clienteNome || "—"}
            />
            <Field
              label="Criado em"
              value={
                oportunidade.createdAt
                  ? new Date(oportunidade.createdAt).toLocaleDateString("pt-BR")
                  : "—"
              }
            />
          </div>
                  ...
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-2.5">
                      <Link
                        ...
                        className="text-sm font-medium text-slate-900 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                      >
                        {p.titulo}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5 text-sm text-slate-700 dark:text-slate-300">
                      {formatarMoedaProposta(p.valor)}
                    </td>
                    <td className="px-4 py-2.5">
                      <span
                        className={`inline-flex text-[10px] px-2 py-0.5 rounded-full font-medium ${PROPOSTA_STATUS[p.status]?.cor || ""}`}
                      >
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-50 flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">
                ({(oportunidade.faturamentos || []).length})
              </span>
            </h2>
            <Link
              ...
              {(oportunidade.faturamentos || []).slice(0, 5).map((f) => (
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    <span
                      className={`inline-flex w-fit text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_FATURAMENTO_CORES[f.status] || ""}`}
                    >
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-50 flex items-center gap-2">
              ...
              Pedidos de Venda
              <span className="text-xs text-slate-400">
                ({(oportunidade.pedidosVenda || []).length})
              </span>
            </h2>
            <Link
              ...
              {(oportunidade.pedidosVenda || []).slice(0, 5).map((p) => (
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    <span
                      className={`inline-flex w-fit text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_PEDIDO_VENDA_CORES[p.status] || ""}`}
                    >

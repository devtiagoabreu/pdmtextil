      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">
            Faturamentos{info && <InfoButton content={info} />}
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            {isLoading
              ? "Carregando..."
              : totalRows > 0
                ? `${fromRow}-${toRow} de ${totalRows} faturamento(s)`
                : "0 faturamentos"}
          </p>
        </div>
        <Link
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {debouncedSearch
                ? "Nenhum faturamento encontrado para essa busca"
                  ...
                  {rows.map((f) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-2 md:px-4 md:py-3">
                        <span
                          className={`inline-flex text-[10px] px-1.5 md:px-2 py-0.5 rounded-full font-medium ${STATUS_FATURAMENTO_CORES[f.status] || ""}`}
                        >

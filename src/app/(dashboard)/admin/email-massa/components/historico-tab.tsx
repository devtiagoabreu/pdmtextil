...
function DisparosSection() {
  ...
  const continuarEnvio = async () => {
    ...
    try {
      ...
      toast.success(
        partes.length ? `Envio processado: ${partes.join(", ")}` : "Nenhum envio pendente na fila"
      )
    } catch {
  ...
  const reenfileirar = async (d: Disparo) => {
    try {
      const res = await fetch(`/api/admin/email-massa/disparos/${d.id}/reenfileirar`, {
        method: "POST",
      })
      if (res.ok) {
      ...
      if (tipo) {
        ...
        exportPDFRelatorio({
          ...
          tables: linhas.length
            ? [
                {
                  title: `${label[tipo]} (${linhas.length})`,
                  headers: headersRelatorio,
                  rows: linhas.map(linhaEnvio),
                },
              ]
            : [],
            ...
            ? [
                {
                  ...
                  rows: data.links.map((l: any) => [l.urlOriginal, l.total]),
                },
              ]
            : []),
        ],
  ...
  const sincronizarBounces = async () => {
    ...
    try {
      ...
      if (data.marcados > 0) {
        toast.success(
          `${data.marcados} bounce(s) identificado(s) e marcado(s) como falha${data.processados ? ` em ${data.processados} notificação(ões) lida(s)` : ""}`
        )
      } else {
    ...
    <section className="flex flex-col space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={sincronizarBounces}
          disabled={sincronizando}
          className="gap-1 active:opacity-70"
        >
          {sincronizando ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}{" "}
          Sincronizar bounces
        </Button>
      </div>
      ...
      {loadingDisparos ? (
        ...
        <div className="flex flex-col space-y-2">
          {disparos.slice(0, 10).map((d) => {
            ...
            const perc = total > 0 ? Math.round((processados / total) * 100) : 0
            const emAndamento =
              d.status === "fila" || d.status === "enviando" || d.status === "pausado"
            return (
              <div
                key={d.id}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-slate-200 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
                    <div
                      className={`h-1.5 rounded-full ${d.status === "pausado" ? "bg-amber-500" : emAndamento ? "bg-blue-500" : d.falhas > 0 ? "bg-amber-500" : "bg-green-500"}`}
                      style={{ width: `${perc}%` }}
                    />
                  </div>
                ...
                <p className="text-xs text-slate-400">
                  {processados} de {total} &middot; {d.enviados} enviados &middot; {d.falhas} falhas
                  &middot; {d.pendentes} na fila
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    variant="outline"
                    size="xs"
                    onClick={() => gerarRelatorio(d)}
                    disabled={relatorioLoading !== null}
                    className="gap-1 active:opacity-70"
                  >
                    {relatorioLoading === d.id ? (
                      <Loader2 size={12} className="animate-spin" />
                    ) : (
                      <FileText size={12} />
                    )}{" "}
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start">
                      <DropdownMenuItem onClick={() => gerarRelatorio(d, "enviados")}>
                        Enviados
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => gerarRelatorio(d, "lidos")}>
                        Lidos
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => gerarRelatorio(d, "cliques")}>
                        Cliques
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => gerarRelatorio(d, "falhas")}>
                        Falhas
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <DropdownMenu>
                    <DropdownMenuTrigger
                      render={
                        <Button variant="outline" size="xs" className="gap-1 active:opacity-70">
                    ...
                    <DropdownMenuContent align="start">
                      <DropdownMenuItem
                        onClick={() => setListaModal({ tipo: "lidos", disparo: d })}
                      >
                        Lidos
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => setListaModal({ tipo: "clicados", disparo: d })}
                      >
                        Cliques
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => setListaModal({ tipo: "falhas", disparo: d })}
                      >
                        Falhas
                      </DropdownMenuItem>
                    </DropdownMenuContent>

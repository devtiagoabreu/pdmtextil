...
const envios = [
  {
    ...
    status: "enviado",
    error: null,
    abertoEm: "2026-07-10T10:00:00.000Z",
...
describe("HistoricoTab", () => {
  it("Continuar envio chama o processar e mostra o resultado", async () => {
    ...
    await waitFor(() =>
      expect(findCall(fetchMock.calls, "/api/admin/email-massa/processar", "POST")).toBeDefined()
    )
    await waitFor(() =>
      expect(toastMock.success).toHaveBeenCalledWith(
        "Envio processado: 5 enviado(s), 1 falha(s), 2 restante(s)"
      )
    )
  })
  ...
  it("Sincronizar bounces chama o endpoint e mostra o resultado", async () => {
    ...
    await waitFor(() =>
      expect(
        findCall(fetchMock.calls, "/api/admin/email-massa/bounces/sincronizar", "POST")
      ).toBeDefined()
    )
    await waitFor(() =>
      expect(toastMock.success).toHaveBeenCalledWith(expect.stringContaining("163 bounce(s)"))
    )
  })
  ...
  it("Sincronizar bounces sem resultado mostra aviso de nada encontrado", async () => {
    ...
    await waitFor(() =>
      expect(toastMock.info).toHaveBeenCalledWith("Nenhum bounce novo encontrado")
    )
  })
  ...
  it("Relatório do card gera PDF com os dados do disparo", async () => {
            ...
            envios: [
              {
                ...
                error: null,
                abertoEm: "2026-07-10T10:00:00.000Z",
                enviadoEm: "2026-07-09T10:00:00.000Z",
                totalCliques: 2,
              },
              {
                id: 2,
    ...
    await waitFor(() =>
      expect(
        findCall(fetchMock.calls, "/api/admin/email-massa/disparos/2/relatorio", "GET")
      ).toBeDefined()
    )
    await waitFor(() =>
      expect(exportPDFRelatorio).toHaveBeenCalledWith(
        expect.objectContaining({ filename: expect.stringContaining("relatorio-disparo-2-") })
      )
    )
    ...
    expect(args.tables.map((t: any) => t.title)).toEqual([
      "Lidos (1)",
      "Enviados (1)",
      "Falhas (1)",
      "Links mais clicados",
    ])
    expect(args.tables[0].headers).toEqual([
      "Email",
      "Nome",
      "Aberto em",
      "Enviado em",
  ...
  it("Criar lista abre o modal de falhas a partir do combobox do disparo e cria a lista via POST", async () => {
    ...
    expect(
      within(dialog).getByRole("heading", { name: /Criar lista de contatos que falharam/ })
    ).toBeInTheDocument()
    ...
    await waitFor(() =>
      expect(
        findCall(fetchMock.calls, "/api/admin/email-massa/disparos/2/criar-lista", "POST")
      ).toBeDefined()
    )
    await waitFor(() =>
      expect(toastMock.success).toHaveBeenCalledWith(expect.stringContaining("1 contato(s)"))
    )
  })

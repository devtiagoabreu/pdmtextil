...
export default function AdminTelasPage() {
  ...
  const [roles, setRoles] = useState<RoleOption[]>(ROLES_FIXAS)
  const [roleSelecionado, setRoleSelecionado] = useState("DEFAULT")
  ...
  async function salvarPaginaInicial() {
    setSaving(true)
    try {
      const res = await fetch("/api/admin/pagina-inicial", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ role: roleSelecionado, paginaInicial }),
      })
  ...
  async function criarMenu() {
    ...
    try {
      const res = await fetch("/api/admin/menus", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ role: roleSelecionado, titulo: titulo.trim(), ordem: menus.length }),
      })
      if (!res.ok) throw new Error()
  ...
  async function salvarItem(menuId: number, itemId: number) {
    ...
    try {
      const res = await fetch(`/api/admin/menus/${menuId}/itens/${itemId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      })
    ...
    <div className="max-w-4xl mx-auto py-8 space-y-6 animate-fade-in">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">
            Configuração de Telas{info && <InfoButton content={info} />}
          </h1>
        </div>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          Gerencie os menus e a página inicial de cada perfil de usuário
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
        <div className="flex items-end gap-3">
          ...
          {roleSelecionado !== "DEFAULT" && (
            <p className="text-xs text-slate-400 pb-1">
              Configurando menus para o perfil <strong>{roleSelecionado}</strong>
            </p>
          )}
        </div>
      </div>

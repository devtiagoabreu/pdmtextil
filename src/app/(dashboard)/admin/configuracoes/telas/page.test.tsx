...
const menus = [
  {
    ...
    icone: null,
    ordem: 1,
    itens: [{ id: 11, titulo: "Clientes", url: "/cadastros/clientes", ordem: 1 }],
  },
]

const telas = [
  { id: "clientes", label: "Clientes", href: "/cadastros/clientes", module: "cadastros" },
]
...
function setup() {
  const fetchMock = createFetchMock(({ method, url }) => {
    ...
    if (method === "GET" && url === "/api/admin/pagina-inicial?role=DEFAULT")
      return { json: { paginaInicial: "" } }
...
describe("AdminTelasPage", () => {
  it("renderiza o heading e os menus do perfil", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Configuração de Telas" }, { timeout: 5000 })
    ).toBeInTheDocument()
  ...
  it("mostra estado vazio de menus", async () => {
    ...
    expect(
      await screen.findByText(/Nenhum menu configurado/, {}, { timeout: 5000 })
    ).toBeInTheDocument()
  })
})

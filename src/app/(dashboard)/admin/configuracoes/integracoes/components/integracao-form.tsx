  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 bg-white dark:bg-slate-900">
      <h2 className="text-lg font-semibold">
        {editItem ? "Editar Integração" : "Nova Integração"}
      </h2>
        ...
        <div className="flex flex-wrap gap-2">
          {(Object.keys(TIPO_AUTH_LABEL) as TipoAuth[]).map((tipo: any) => (
            <button
              ...
              onClick={() => {
                setTipoAuth(tipo)
                setAuthConfigJson(getAuthPlaceholder(tipo))
              }}
      ...
      <div className="space-y-2">
        ...
        {showJson && (
          <p className="text-xs text-slate-400">Exemplo: {getAuthPlaceholder(tipoAuth)}</p>
        )}
        <textarea
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="clientes, fios, bases-urdume"
        />
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            disabled={!editItem || loadingFields}
        ...
        {apiFields.length > 0 ? (
            ...
            <div className="border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800/50">
                  <tr>
                    <th className="p-2 text-left text-xs font-medium text-slate-500 uppercase w-1/2">
                      Campo da API
                    </th>
                    <th className="p-2 text-left text-xs font-medium text-slate-500 uppercase w-1/2">
                      Campo PDM
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {apiFields.map((f: any) => (
                    <tr key={f}>
                      <td className="p-2 text-xs font-mono text-slate-700 dark:text-slate-300">
                        {f}
                      </td>
                      <td className="p-2">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setMappingJson(
                              JSON.stringify(
                                { fields: newMappings, uniqueKey: uniqueKeyField },
                                null,
                                2
                              )
                            )
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setMappingJson(
                    JSON.stringify({ fields: fieldMappings, uniqueKey: e.target.value }, null, 2)
                  )
                }}
          ...
          <div className="border border-dashed border-slate-200 dark:border-slate-800 rounded-lg p-6 text-center">
            <p className="text-xs text-slate-400">
              Clique em &quot;Carregar campos da API&quot; para ver os campos do retorno e

...
const SolicitacoesCriadasCharts = dynamic(
  () => import("./charts").then((m) => m.SolicitacoesCriadasCharts),
  { ssr: false }
)
...
export default function RelatorioSolicitacoesCriadas() {
  ...
  function handleExportCSV() {
    exportCSV(
      ...
      ["Mês", "Criadas", "Deletadas", "Concluídas"],
      porMes.map((m: any) => [m.mes, m.criadas, m.deletadas, m.concluidas])
    )
    setTimeout(() => {
      exportCSV(
        "solicitacoes-recentes",
        ["#", "Cliente", "Tipo", "Status", "Criado em"],
        recentes.map((r: any) => [
          r.id,
          r.cliente,
  ...
  async function handleExportPDF() {
    await exportPDFRelatorio({
      ...
      tables: [
        {
          headers: ["Mês", "Criadas", "Deletadas", "Concluídas"],
          rows: porMes.map((m: any) => [m.mes, m.criadas, m.deletadas, m.concluidas]),
        },
        {
          headers: ["#", "Cliente", "Tipo", "Status", "Criado em"],
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">
          Relatório: Solicitações de Desenvolvimento Criadas / Deletadas
          {info && <InfoButton content={info} />}
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          Acompanhe o volume de solicitações de desenvolvimento criadas, deletadas, concluídas e a
          taxa de sucesso
        </p>
      </div>
                  ...
                  >
                    ...
                    <td className="p-3">
                      <span
                        ...
                        style={{
                          backgroundColor: hexToRgba(getStatusColor(r.status), 0.12),

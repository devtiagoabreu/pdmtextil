...
const SolicitacoesConcluidasCharts = dynamic(
  () => import("./charts").then((m) => m.SolicitacoesConcluidasCharts),
  { ssr: false }
)
...
export default function RelatorioSolicitacoesConcluidas() {
  ...
  const { data, isLoading: loading } = useQuery<any>({
    queryKey: ["relatorio-solicitacoes-concluidas", aplicadoDataInicio, aplicadoDataFim],
    queryFn: async () => {
      ...
      const r: any = await fetch(`/api/relatorios/solicitacoes-concluidas?${params}`)
      return r.json()
    },
  })
  ...
  function handleExportCSV() {
    setTimeout(() => {
      exportCSV(
        "concluidas-desenvolvimento",
        ["#", "Cliente", "Projeto", "Tipo", "Criado em", "Concluído em", "Dias em Dev"],
        lista.map((r: any) => [
          r.id,
          r.cliente,
          r.projeto || "-",
  ...
  async function handleExportPDF() {
    await exportPDFRelatorio({
      ...
      stats: stats
        ...
        : undefined,
      tables: [
        { headers: ["Mês", "Concluídas"], rows: porMes.map((m: any) => [m.mes, m.concluidas]) },
        {
          headers: ["#", "Cliente", "Projeto", "Tipo", "Criado em", "Concluído em", "Dias"],
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">
          Relatório: Concluídas Desenvolvimento{info && <InfoButton content={info} />}
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          Solicitações de desenvolvimento com status Concluído Desenvolvimento — total, distribuição
          por tipo e detalhamento
        </p>
      </div>
      ...
      <SolicitacoesConcluidasCharts porMes={porMes} />

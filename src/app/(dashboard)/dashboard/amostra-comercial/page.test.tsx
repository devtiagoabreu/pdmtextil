...
describe("DashboardAmostraComercial", () => {
  it("renderiza o heading e o estado vazio de requisições recentes", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Dashboard — Amostras Comerciais/ })
    ).toBeInTheDocument()
  ...
  it("abre o modal de filtro como dialog acessível ao clicar num card", async () => {
    const fetchMock = createFetchMock(
      routeJson({
        ...
        "GET /api/dashboard/amostra-comercial-lista?filtro=pendentes": [
          {
            id: 5,
            titulo: "Amostra Verão",
            cliente: "Malharia Tupiniquim",
            status: "PENDENTE",
    ...
    await waitFor(() =>
      expect(screen.queryByRole("dialog", { name: "Pendentes" })).not.toBeInTheDocument()
    )
  })
})

...
const ReqCorteCharts = dynamic(() => import("./charts").then((m) => m.ReqCorteCharts), {
  ssr: false,
})
            ...
            {[
              {
                ...
                value: stats?.totalCortes ?? 0,
                color: "text-purple-600 dark:text-purple-400",
                bg: "bg-purple-50 dark:bg-purple-950/50",
              },
              {
                label: "Qtd Total (Itens)",
              ...
              <div
                key={stat.label}
                className={`rounded-xl border border-slate-200 dark:border-slate-800 ${stat.bg} p-3 card-hover`}
              >
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                  {stat.label}
                </p>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-slate-900 dark:text-slate-200 truncate">
                          #{item.id} — {item.requisitanteNome || "—"} ({item.totalCortes} cortes,
                          qtd {item.quantidadeTotal})
                        </p>
                      </div>
                      <div className="flex items-center gap-3 ml-3 shrink-0">
                        <span
                          className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_BG[item.status] || "bg-slate-100 text-slate-600"}`}
                        >

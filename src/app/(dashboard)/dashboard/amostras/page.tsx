...
const AmostrasCharts = dynamic(() => import("./charts").then((m) => m.AmostrasCharts), {
  ssr: false,
})
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">
            Dashboard de Amostras de Desenvolvimento{info && <InfoButton content={info} />}
          </h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <AmostrasCharts stats={stats} getLabel={getLabel} getColor={getColor} />
              ...
              {!stats?.recent || stats.recent.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                    Nenhuma amostra recente
                  </p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                    As amostras aparecerão aqui
                  </p>
                </div>
              ) : (
                <table className="w-full">
                  <thead className="border-b border-slate-200 dark:border-slate-800">
                    <tr>
                      ...
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 p-4 w-28">
                        Ação
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.recent.map((a: any, i: number) => (
                      <tr
                        key={`${a.tipoAmostra}-${a.id}-${i}`}
                        className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50"
                      >
                        <td className="p-4 text-sm">
                          <Link
                            href={`/cadastros/produto-cru/${a.produtoId}?tab=amostras`}
                        ...
                        <td className="p-4 text-sm">
                          {a.solicitacaoId ? (
                            <Link
                              href={`/comercial/solicitacoes/${a.solicitacaoId}`}
                              className="text-blue-600 dark:text-blue-400 hover:underline text-xs"
                            >
                              {a.solicitacaoCliente || `#${a.solicitacaoId}`}
                              {a.solicitacaoProjeto ? ` — ${a.solicitacaoProjeto}` : ""}
                            </Link>
                          ) : (
                            <span className="text-slate-300">—</span>
                          )}
                        ...
                        <td className="p-4 text-sm">
                          <span
                            className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${TIPO_BG[a.tipoAmostra] || "bg-slate-100 text-slate-600"}`}
                          >
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-slate-900 dark:text-slate-200 truncate">
                          {item.produtoCodigo || `#${item.produtoId}`} —{" "}
                      ...
                      <div className="flex items-center gap-3 ml-3 shrink-0">
                        <span
                          className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${TIPO_BG[item.tipoAmostra] || "bg-slate-100 text-slate-600"}`}
                        >
